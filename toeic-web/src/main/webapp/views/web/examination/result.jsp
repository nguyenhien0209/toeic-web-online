        ...
        <div style="overflow: auto; height: 500px" >
            <c:forEach var="item" items="${items.listResult}">
                <c:if test="${not empty item.correctAnswer}">
                    ${item.number}.&nbsp;&nbsp;&nbsp;
                    A <input type="radio" name="answerUser[${item.examinationQuestionId}]" value="A" ${item.answerUser == 'A' ? 'checked':''} disabled="">
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    B <input type="radio" name="answerUser[${item.examinationQuestionId}]" value="B" ${item.answerUser == 'B' ? 'checked':''} disabled>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    C <input type="radio" name="answerUser[${item.examinationQuestionId}]" value="C" ${item.answerUser == 'C' ? 'checked':''} disabled>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    D <input type="radio" name="answerUser[${item.examinationQuestionId}]" value="D" ${item.answerUser == 'D' ? 'checked':''} disabled>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <br/>
                    <br/>
                </c:if>
